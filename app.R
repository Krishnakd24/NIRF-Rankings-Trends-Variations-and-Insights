  ...
  (
    ...
    mainPanel(
      ...
      conditionalPanel(
        ...
        tabsetPanel(
          ...
          tabPanel(" Institute-wise Score Trends",
                   br(),
   ...
   and compare states or institutions on multiple parameters."),
        ...
        tags$ul(
          ...
          tags$li("Place all yearly NIRF .csv files in the same directory as of app."),
...
server = function(input, output, session) {
  ...
  data_files = list(
    "2017" = "2017_overall.csv", 
    "2018" = "2018_overall.csv",
    "2019" = "2019_overall.csv",
    "2020" = "2020_overall.csv",
    "2021" = "2021_overall.csv",
    "2022" = "2022_overall.csv",
    "2023" = "2023_overall.csv",
    "2024" = "2024_overall.csv",
    "2025" = "2025_overall.csv" 
  )
  ...
  # FOR year tabs
  observeEvent(input$y_17, { updateTextInput(session, "selected_year", value = "2017") })
  observeEvent(input$y_18, { updateTextInput(session, "selected_year", value = "2018") })
  observeEvent(input$y_19, { updateTextInput(session, "selected_year", value = "2019") })
  observeEvent(input$y_20, { updateTextInput(session, "selected_year", value = "2020") })
  observeEvent(input$y_21, { updateTextInput(session, "selected_year", value = "2021") })
  observeEvent(input$y_22, { updateTextInput(session, "selected_year", value = "2022") })
  observeEvent(input$y_23, { updateTextInput(session, "selected_year", value = "2023") })
  observeEvent(input$y_24, { updateTextInput(session, "selected_year", value = "2024") })
  observeEvent(input$y_25, { updateTextInput(session, "selected_year", value = "2025") })
  ...
  output$lpop <- renderPlot({
    ...
    ggplot(plot_data, aes(x = Score, y = reorder(Name, Score))) +
      ...
      labs(title = paste("Institute Rankings -", input$selected_year),
  ...
  output$tlrhist = renderPlot({
    ...
    ggplot(data, aes(x = TLR)) +
      ...
      labs(title = "Distribution of TLR Scores",
  ...
  output$rpchist = renderPlot({
    ...
    ggplot(data, aes(x = RPC)) +
      ...
      labs(title = "Distribution of RPC Scores",
  ...
  output$gohist <- renderPlot({
    ...
    ggplot(data, aes(x = GO)) +
      ...
      labs(title = "Distribution of GO Scores",
  ...
  output$oihist <- renderPlot({
    ...
    ggplot(data, aes(x = OI)) +
      ...
      labs(title = "Distribution of OI Scores",
  ...
  output$perceptionhist <- renderPlot({
    ...
    ggplot(data, aes(x = Perception)) +
      ...
      labs(title = "Distribution of Perception Scores",
  ...
  output$collegesperstate <- renderPlot({
    data <- loaded_data()
    state_summary <- data %>%group_by(State) %>%summarise(
      Colleges = n(),
      .groups = 'drop'
    ) %>%arrange(desc(Colleges))
  ...
  output$instituteplot <- renderPlot({
    ...
    ggplot(complete_data, aes(x = Year, y = Score, group = 1)) +
      geom_line(color = "darkgreen", linewidth = 2) + geom_point(color = "darkgreen", size = 4) +
  ...
  output$overall_yearly_plot <- renderPlot({
    ...
    ggplot(yearly_avg, aes(x = Year, y = AvgScore, group = 1)) + geom_line(color = "blue", linewidth = 2) +
